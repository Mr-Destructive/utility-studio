...
export const ThemedSiderV2: React.FC<RefineThemedLayoutV2SiderProps> = ({
  ...
  const renderTreeView = (tree: ITreeMenu[], selectedKey?: string) => {
    return tree.map((item: ITreeMenu) => {
      let { icon, label, route, name, children, parentName, meta, options } =
        item;
        let color = 'white';
      const isOpen = open[item.key || ""] || false;
      if (label === "Studio.Studio"){
        icon = <AppIcon/>
        label = "Studio"
      } else if (label === "YouTube.YouTube"){
        icon = <YouTubeIcon/>
        color = "red";
        label = "YouTube"
      } else if (label === "Documents.Documents"){
        icon = <DescriptionIcon/>
        color = "#1e90ff";
        label = "Documents"
      } else if (label === "Links.Links"){
        icon = <LinkIcon/>
        color = "#00bfff";
        label = "Links";
